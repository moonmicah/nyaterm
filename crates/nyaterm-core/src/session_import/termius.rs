...
pub(super) fn parse_termius_indexed_db(
    path: Option<&Path>,
    local_key_secret: &[u8],
) -> AppResult<PreparedJsonImport> {
    let db_path = resolve_termius_indexed_db_path(path)?;
    let local_key = normalize_termius_local_key(local_key_secret)?;
...
fn resolve_termius_indexed_db_path(path: Option<&Path>) -> AppResult<PathBuf> {
    let resolved = if let Some(path) = path {
        normalize_termius_indexed_db_path(path.to_path_buf())
    } else {
...
fn normalize_termius_local_key(secret: &[u8]) -> AppResult<Zeroizing<[u8; 32]>> {
    normalize_termius_local_key_bytes(secret)
}
...
fn copy_leveldb_dir(source: &Path, target: &Path) -> AppResult<()> {
    std::fs::create_dir_all(target).map_err(|error| {
        AppError::Config(format!("Cannot prepare Termius IndexedDB copy: {error}"))
    })?;
    for entry in std::fs::read_dir(source).map_err(|error| {
        AppError::Config(format!("Cannot read Termius IndexedDB directory: {error}"))
    })? {
        let entry = entry.map_err(|error| {
            AppError::Config(format!("Cannot read Termius IndexedDB entry: {error}"))
        })?;
        let source_path = entry.path();
        let target_path = target.join(entry.file_name());
        if source_path.is_file() {
            std::fs::copy(&source_path, &target_path).map_err(|error| {
                AppError::Config(format!("Cannot copy Termius IndexedDB file: {error}"))
            })?;
        }
    }
    Ok(())
...
mod tests {
    ...
    fn decrypts_termius_encrypted_values_with_local_key() {
        let key = [7_u8; 32];
        let nonce = [3_u8; 24];
        let encrypted = encrypt_termius_secret_for_test("secret-value", &key, &nonce);
        let mut values = vec![TermiusTaggedValue::String(encrypted)];

        decrypt_termius_values(&mut values, &key).expect("decrypt values");

        assert_eq!(
            values,
            vec![TermiusTaggedValue::String("secret-value".to_string())]
        );
    }

    #[test]
    fn prepares_connections_groups_passwords_and_keys() {
        let prepared = prepare_termius_import(TermiusRawStore {
            groups: vec![TermiusRawGroup {
            ...
            hosts: vec![TermiusRawHost {
                ...
                username: None,
                password: None,
                ssh_config_id: Some("config-id".to_string()),
                identity_id: None,
        ...
        assert_eq!(connection.name, "Prod web");
        assert_eq!(connection.group_path, Some(vec!["Production".to_string()]));
        assert!(matches!(
            &connection.config,
