...
pub(super) fn parse_json_import(path: &Path) -> AppResult<PreparedJsonImport> {
    let content = read_text_file_limited(path, "JSON session import file")?;
    parse_json_import_content(&content)
}

fn parse_json_import_content(content: &str) -> AppResult<PreparedJsonImport> {
    let value: serde_json::Value = serde_json::from_str(content)
        .map_err(|error| AppError::Config(format!("Invalid JSON session import file: {error}")))?;
...
fn is_electerm_bookmarks_json(value: &serde_json::Value) -> bool {
    value.as_object().is_some_and(|object| {
        object.contains_key("bookmarks") && object.contains_key("bookmarkGroups")
    })
}
